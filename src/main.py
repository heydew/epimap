...
GEOJSON_PATH = Path(__file__).resolve().parents[1] / "data" / "world_countries.geojson"
...
if __name__ == "__main__":
    ...
    print(f"ok, donnees COVID chargees ({date_min} -> {date_max})")
    print(f"{n_pays} pays, population mondiale ~{pop_G:.1f}G")
    print(f"pic infectes : {world['I'].max():,.0f}  ({world.loc[world['I'].idxmax(), 'date'].date()})")
